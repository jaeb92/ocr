...
from jamo_custom import get_jamo_type, split_syllable_char
import numpy as np
...
r = []
# test_target2 = list(map(lambda x: ''.join(y) for y in x, test_university))
for i in range(len(test_university)):
    a = []
    for j in range(len(test_university[i])):
        # print(test_university[i][j])
        a.append(split_syllable_char(test_university[i][j]))
        # print(split_syllable_char(test_university[i][j]))
    r.append(a)
